...
<body>
                    ...
                    <div class="card-body">
                        <blockquote class="blockquote mb-0">
                            <div class="form">
                                ...
                                <form autocomplete="off" method="POST" action="{{route('admin.proposal.edit',$proposal->proposal_id) }}" enctype="multipart/form-data">
                                    @csrf
                                    @if ($errors->any())
    ...
    <script>
        (()=>{
            $proposal = <?php echo $proposal ?>;
            var title = document.getElementById("title-proposal");
            var abstract = document.getElementById("abstract");
            var amount = document.getElementById("amount");
            var submissionDate = document.getElementById("submission-date");
            var proposal_file = document.getElementById("proposal_file");
            var facultyCount = document.getElementById("number_of_faculty");
            var faculty_group = <?php echo  $faculty_group ?>; 
            var dept = document.getElementById("dept");

            console.log(faculty_group)
            console.log(faculty_group["faculty_group_department"])

            facultyCount.value = faculty_group["faculty_group_department"].split(",").length;
            facultyCount.onchange();
            facultyCount=facultyCount.value;
            var faculty_department = faculty_group["faculty_group_department"].split(",");
            var faculty_name = faculty_group["faculty_group_name"].split(",");
            var faculty_id =[];
            // get id from php faculty name
            <?php
            $faculty_name = explode(",",$faculty_group["faculty_group_name"]);
            foreach ($faculty_name as $faculty_name) {
                $faculty = DB::table('faculties')->get()->where('faculty_name',$faculty_name);
                foreach ($faculty as $faculty) {
                    echo "console.log('faculty id',$faculty->faculty_id);";
                    echo "faculty_id.push($faculty->faculty_id);";
                }
            }
            ?>
            
            
            var faculty_department_input = [];
            for(var i=1;i<=facultyCount;i++){
                faculty_department_input.push(document.getElementById("faculty_department_"+i));
            }
            var faculty_name_input = [];
            for(var i=1;i<=facultyCount;i++){
                faculty_name_input.push(document.getElementById("faculty_name_"+i));
            }
            var proposal_group_id = $proposal.proposal_group_id;
            // get faculty group from database
            
            var dept = document.getElementById("dept");

            // fille the input fields
            title.value = $proposal.proposal_title;
            abstract.value = $proposal.proposal_abstract;
            amount.value = $proposal.proposal_fundingAmount;
            submissionDate.value = $proposal.proposal_submissionDate;
            facultyCount.value = $proposal.proposal_facultyCount;
            
            for(var i=1;i<=facultyCount;i++){
                faculty_department_input[i-1].value = faculty_department[i-1];
                faculty_department_input[i-1].onchange();
            }
            
            setTimeout(()=>{
                for(var i=1;i<=facultyCount;i++){
                    faculty_name_input[i-1].value = faculty_id[i-1];
                }
            },1500);  
